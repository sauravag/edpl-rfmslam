function pointsToPlot = error_ellipse(varargin)
% ERROR_ELLIPSE - Get points to plot for an error ellipse, or ellipsoid, defining confidence region
...
if r==3 & c==3
    error('Saurav: Code would need some minor adjustments for 3D')
    z0=mu(3);
    ...
    [x,y,z] = getpoints(Cxy,npoints,prop.clip);
    h1=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style);hold on
    [y,z,x] = getpoints(Cyz,npoints,prop.clip);
    h2=plot3(x0+k*x,y0+k*y,z0+k*z,prop.style,npoints);hold on
    [z,x,y] = getpoints(Czx,npoints,prop.clip);
...
elseif r==2 & c==2
    ...
    [x,y,z] = getpoints(C,npoints,prop.clip);
    
    %   set(h,'xdata',x,'ydata',y,'zdata',z+1)
    if nargout
        pointsToPlot = [(scale*(x0+k*x))' NaN;(scale*(y0+k*y))' NaN];
    else
...
%   x and y.
function [x,y,z] = getpoints(C,n,clipping_radius)

p=0:2*pi/n:2*pi; % angles around a circle
